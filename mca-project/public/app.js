        ...
        app.client.request(undefined, 'api/menu', 'GET', queryStringObject, undefined, function (statusCode, responsePayload) {
            if (statusCode == 200 && responsePayload) {
                ...
                for (var key in pizzas) {
                    ...
                    td1.innerHTML = pizzas[key].ingredients;
                    td2.innerHTML = pizzas[key].price + '$';
                    td3.innerHTML = '<button class="addPizza" onclick="app.addToCart(\'' + email + '\', \'' + key + '\')">Add a ' + key + ' pizza </button>';
                }
            } else {
    ...
    if (email) {
        ...
        app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined, function (statusCode, responsePayload) {
            if (statusCode == 200 && responsePayload) {
                ...
                var usersShoppingCart = responsePayload.shoppingCart;
                var shoppingCart = document.getElementById("shoppingCart");

                if (usersShoppingCart.pizzas.length > 0) {
                    ...
                    usersShoppingCart.pizzas.forEach(pizza => {
                        var item = document.createElement("li");
                        item.appendChild(document.createTextNode(pizza));
...
app.addToCart = function (email, pizza) {
    // Fetch the user data
    var queryStringObject = {
        'email': email
    };
    ...
    app.client.request(undefined, 'api/menu', 'POST', queryStringObject, payload, function (statusCode, responsePayload) {
        if (statusCode == 200) {
            // Add the pizza in the shopping cart list
            var shoppingCart = document.getElementById("shoppingCart");
            var total = document.getElementById("total");
            ...
            item.appendChild(document.createTextNode(pizza));
            shoppingCart.appendChild(item);
            total.innerHTML = responsePayload.total;
